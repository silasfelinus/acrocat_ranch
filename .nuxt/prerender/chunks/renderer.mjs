...
import {
  stringify,
  uneval
} from 'file:///home/silasfelinus/code/kindrobots/node_modules/devalue/index.js'
...
import {
  ...
  g as getRouteRules
} from './nitro/nitro-prerenderer.mjs'
import { joinURL } from 'file:///home/silasfelinus/code/kindrobots/node_modules/ufo/dist/index.mjs'
...
const getEntryId = () =>
  getClientManifest().then((r) => Object.values(r).find((r2) => r2.isEntry).src)
const getStaticRenderedHead = () =>
  import('./rollup/_virtual_head-static.mjs').then((r) => r.default || r)
...
const renderer = defineRenderHandler(async (event) => {
  ...
  const ssrContext = {
    ...
    noSSR:
      event.context.nuxt?.noSSR || routeOptions.ssr === false || PRERENDER_NO_SSR_ROUTES.has(url),
    error: !!ssrError,
    nuxt: void 0,
    /* NuxtApp */
...
function renderPayloadJsonScript(opts) {
  ...
  return `<script ${attrs.join(
    ' '
  )}>${contents}<\/script><script>window.__NUXT__={};window.__NUXT__.config=${uneval(
